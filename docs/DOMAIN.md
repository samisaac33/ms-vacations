...
| Ciudad | Portoviejo | Casas vacacionales |
...
**Pago fraccionado (`payment_timing: split`):** el huésped paga 50 % al reservar y el saldo antes del check-in (mínimo 7 días de anticipación). Tras el anticipo, el estado pasa a `pending_balance` hasta liquidar el saldo.
...
- Temporadas altas configurables en `/admin/configuracion`: intervalos por propiedad con mínimo de noches adicional. Aplica si **alguna noche** de la estancia cae en el intervalo (no solo el check-in); se exige el mayor entre la regla base y la de temporada
- La noche del **31 de diciembre** cuesta el **doble** del precio huésped habitual (todas las propiedades), salvo override manual en calendario admin
